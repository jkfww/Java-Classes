...
/* ------- apply liniowa --------*/
    ...
    public void apply(Applayable a, String name, int njobs){
        ...
        ApplayableThread [] tasks = new ApplayableThread[njobs+1];
        ...
        {
            for(int j=0; j<tasks[i].result.values.size(); j++)
            {
                result.addValue(tasks[i].result.values.get(j).toString());
            }

        }
        this.columns.set(columnNumber, result);
